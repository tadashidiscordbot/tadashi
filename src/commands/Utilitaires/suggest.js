...
module.exports = {
    ...
    example: "/suggest `[suggestion: Invite tadashi dans ce serveur]`",
    category: "⚙️ Utilités",
    options: [
        {
            name: "suggestion",
    ...
    async runInteraction(client, interaction) {
        ...
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder()
                ...
                .setEmoji(`<:greentick:1025082258022993950>`)
                .setStyle(ButtonStyle.Success)
                .setLabel("Accepter"),
            new ButtonBuilder()
                .setCustomId("suggest-decline")
                .setEmoji(`<:redtick:1025082292667961364>`)
                .setStyle(ButtonStyle.Danger)
                .setLabel("Refuser")
        );

        const channelData1 = await channelData.findOne({ Guild: guild.id })
        if(!channelData1) return Error(interaction, "Les modérateurs ou les administrateurs n'ont pas configurés le salon des suggestion, préviens l'équipe du staff de ton serveur de le configurer ! Dis a tes modérateurs de faire la commande slash `/config suggest` de Tadashi !")
